...
game.events.on("collision", (([ c, c_, d ]) => {
  ...
  return (function() {
    if (c.entity === target) {
      ...
      console.log("before target collided with ant", hpos, c_, c_v);
      c_v.pos.x = (30 + hpos.x);
      c_v.pos.y = (30 + hpos.y);
      return console.log("after target collided with ant", hpos, c_, c_v);
    } else if (c_.entity === target) {
      var hpos = game.systems.get(Position, home);
      console.log("before collided with target", hpos, c, cv);
      cv.pos.x = (30 + hpos.x);
      cv.pos.y = (30 + hpos.y);
...
(function() {
  ...
  $for = (function() {
    ...
    game.systems.get(Position, dot).x = (homePos.x + (Math.floor((Math.random() * (30 - 1))) + 1));
    game.systems.get(Position, dot).y = (homePos.y + (Math.floor((Math.random() * (30 - 1))) + 1));
    ...
    game.systems.get(Position, dot).x = (200 + (Math.floor((Math.random() * ((config.dimensions[0] / 4) - 1))) + 1));
    game.systems.get(Position, dot).y = (200 + (Math.floor((Math.random() * ((config.dimensions[1] / 4) - 1))) + 1));
    game.systems.get(Position, dot).z = 1;
