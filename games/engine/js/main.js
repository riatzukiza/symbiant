...
(function() {
  ...
  $for = (function() {
    ...
    game.systems.get(Position, dot).x = (Math.floor((Math.random() * ((2 * (config.dimensions[0] / 4)) - 1))) + 1);
    game.systems.get(Position, dot).y = (Math.floor((Math.random() * ((2 * (config.dimensions[1] / 4)) - 1))) + 1);
    game.systems.get(Position, dot).z = 1;
