...
game.events.on("collision", (([ c, c_, d ]) => {
  ...
  return (function() {
    if (c.entity === target) {
      ...
      c_v.xd = 0;
      c_v.yd = 0;
      updateParticle(c_v, c_v.pos, SignalField.field, SignalField.layer, game.ticker.ticks, true);
      ...
      cv.pos.x = (3 + hpos.x);
      cv.pos.y = (3 + hpos.y);
      updateParticle(cv, cv.pos, SignalField.field, SignalField.layer, game.ticker.ticks, true);
...
const ants=[];
var spawnAnt = (function spawnAnt$(x_y$34, home, startingLife) {
  /* spawn-ant eval.sibilant:198:0 */
