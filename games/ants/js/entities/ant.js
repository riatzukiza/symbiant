...
const Ant = extend(Entity, { 
  ...
  _eat( group = this.group ){ 
      ...
      this.life = (this.life + (10 * this._food.life));
...
const Colony = extend(EntityGroup, { 
  ...
  entityType:Ant,
  id:0,
  init( nest = this.nest,color = this.color,goals = this.goals,decay = 0.0001,colonies = this.colonies,foodWeights = create(Pheremones)(color, decay),matingWeights = create(Pheremones)({ 
    red:color.red,
    green:color.green,
    blue:255
  ...
  update( entities = this.entities,weights = this.weights,decay = this.decay,matingWeights = this.matingWeights,foodWeights = this.foodWeights ){ 
      ...
      return (function() {
        if (this.entities.size === 0) {
          ...
          (function() {
            if (this.layer) {
              return sim.layers.remove(this.layer);
            }
          }).call(this);
