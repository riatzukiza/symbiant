...
const Ant = extend(Entity, { 
  ...
  update( group = this.group,nest = this.nest,life = this.life ){ 
      ...
      return this.seeking.emit(this.pos, this.seeking.weights, (this.genetics.rate * (function() {
        if (1 > v) {
          return 1;
        } else {
          return v;
...
const Colony = extend(EntityGroup, { 
  ...
  entityType:Ant,
  id:0,
  init( nest = this.nest,color = this.color,goals = this.goals,decay = 0.00001,colonies = this.colonies,foodWeights = create(Pheremones)(color, decay),matingWeights = create(Pheremones)({ 
    red:color.red,
    green:color.green,
    blue:255
