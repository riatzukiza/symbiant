          ...
          return (function() {
            if (sated__QUERY) {
              return (function() {
                ...
                } else if ((group.colonies.size < 30 && Math.random() > 0.99999)) {
...
const Colony = extend(EntityGroup, { 
  ...
  colonies:(new Set()),
  entityType:Ant,
  init( nest = this.nest,color = this.color,goals = this.goals,decay = 0.9,colonies = this.colonies,foodWeights = create(Pheremones)(color, decay, sim.layers.get()),matingWeights = create(Pheremones)({ 
    red:color.red,
    green:color.green,
    blue:255
