...
const Ant = extend(Entity, { 
  ...
  init( pos = this.pos,color = this.color,life = this.life,genetics = lit(deviance((function() {
    /* eval.sibilant:33:8 */
  
    let rand = ((Math.random() * (0.1 - 0)) + 0);
    return (0.1 - (rand / 2));
  }).call(this)), rate, mutationFactor, findRate((function() {
    /* eval.sibilant:33:8 */
  
    let rand = ((Math.random() * (1 - 0)) + 0);
    return (1 - (rand / 2));
  }).call(this)), returnRate((function() {
    /* eval.sibilant:33:8 */
  
    let rand = ((Math.random() * (1 - 0)) + 0);
    return (1 - (rand / 2));
  }).call(this)), kernel(mooreNeighborhood(3, 3).dmap(() => {
  ...
  _eat( group = this.group ){ 
      ...
      this.life = (this.life + this._food.life);
  ...
  update( group = this.group,nest = this.nest,life = this.life ){ 
      ...
      return this.seeking.emit(this.pos, this.seeking.weights, (this.genetics.rate * (this.life / Ant.life)), 10);
