...
const Ant = extend(Entity, {
    ...
    _reproduce(nest = this.nest, ant = this.ant, group = this.group) {
        ...
        return this
            ...
            .matingWeights
            .emit(ant.pos,
                  group.matingWeights.weights,
                  (10 * ant.genetics.rate * (ant.life / Ant.life)),
                  20
                 );
    ...
    mutate(ant = this.ant, group = this.group, nest = this.nest) {
        ...
        ant.genetics.findRate = (ant.genetics.findRate + (function() {
            /* eval.sibilant:33:8 */

            let rand = ((Math.random() * (ant.genetics.mutationFactor - 0)) + 0);
            return (ant.genetics.mutationFactor - (rand / 2));
        }).call(this));
        ant.genetics.deviance = (ant.genetics.deviance + (function() {
            /* eval.sibilant:33:8 */

            let rand = ((Math.random() * (ant.genetics.mutationFactor - 0)) + 0);
            return (ant.genetics.mutationFactor - (rand / 2));
        }).call(this));
        ant.genetics.rate = (ant.genetics.rate + (function() {
            /* eval.sibilant:33:8 */
...
const Colony = extend(EntityGroup, {
    ...
    init(nest = this.nest, color = this.color, goals = this.goals, decay = 0.1, colonies = this.colonies, foodWeights = create(Pheremones)(color, decay), matingWeights = create(Pheremones)({
        ...
        this.matingWeights = matingWeights;
        this.event = event;
        this.ants = ants;
    ...
    spawn(color = this.color, entityType = this.entityType) {
        ...
        let ent = entityType.spawn((this.nest.x + (Math.floor((Math.random() * (30 - 1))) + 1)),
                                   (this.nest.y + (Math.floor((Math.random() * (30 - 1))) + 1)),
                                   color);
        (function() {
            if (ent) {
                ent.group = this;
